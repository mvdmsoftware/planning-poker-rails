class RoomsController < ApplicationController
   ...
   def show
      ...
      # Error handling
      return redirect_to join_room_path(room_identifier: room_identifier) if (@room.nil?)
      return redirect_to join_room_path(room_identifier: room_identifier) unless (@room.participants.exists?(user_session_id: session_id))
   end
   ...
   def join
      ...
      if (room.nil?)
         room = Room.new({ unique_identifier: room_identifier, is_hidden: true })
         room.save
      end

      participant = room.participants.where(user_session_id: session_id).first
      if participant.nil?
         participant = room.participants.new(user_session_id: session_id)
         room.save
      end

      respond_to do |format|
         format.turbo_stream { render turbo_stream: turbo_stream.append(participant) }
         format.html { redirect_to show_room_path(room_identifier: room_identifier) }
      end
   end
